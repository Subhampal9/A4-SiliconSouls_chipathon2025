...
def two_pfet_interdigitized(
    ...
    ****NOTE: These are the same as glayout.flow.primitives.fet.multiplier arguments EXCLUDING dummy, sd_route_extension, and pdk options
    ...
    base_multiplier.add_padding(
        ...
        default=pdk.get_grule("nwell", "active_diff")["min_enclosure"],
    )
    # add substrate tap
...
#place=add_two_int_labels(two_pfet_interdigitized(pdk=sky130_mapped_pdk,numcols=3,with_substrate_tap=False,width=2,length=1,fingers=2),sky130_mapped_pdk)
#place.show()
#print(place.info['netlist'].generate_netlist())
#place.name = "two_trans_interdigitized"
#netgen_lvs_result = sky130_mapped_pdk.lvs_netgen(place, place.name)
